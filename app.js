...
async function startScan(target, profile) {
  ...
  setTimeout(() => {
    try {
      const data = JSON.parse(JSON.stringify(demoReport)); // Clone demo data
      data.target = target;
      data.hostname = target.replace(/^https?:\/\//, "").split("/")[0];
      data.scanned_at = new Date().toISOString();
      
      renderStatus(`${translations[currentLang].securityAssessment}: ${data.summary.posture || "Secure"}`, data.summary.score);
      renderReport(data);
      finishProgress();
    } catch (error) {
  ...
  }, 5000); // 5 seconds delay to simulate scanning
}
